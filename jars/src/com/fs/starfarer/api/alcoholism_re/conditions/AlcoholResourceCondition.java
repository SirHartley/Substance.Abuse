package com.fs.starfarer.api.alcoholism_re.conditions;

import com.fs.starfarer.api.alcoholism_re.industry.Brewery;
import com.fs.starfarer.api.alcoholism_re.memory.AlcoholAPI;
import com.fs.starfarer.api.alcoholism_re.memory.AlcoholRepo;
import com.fs.starfarer.api.alcoholism_re.memory.FactionAlcoholHandler;
...
public class AlcoholResourceCondition extends BaseMarketConditionPlugin {
    ...
    private void unapplyNPC() {
        ...
        if (market.hasIndustry(Industries.LIGHTINDUSTRY)) {
            ...
            for (IndustrialAlcohol industrialAlcohol : getStaticAlcoholForMarket()) {
                lightIndustry.supply(getModId(), industrialAlcohol.getCommodityId(), 0, "");
    ...
    private void applyAllNPCAlcohol() {
        for (IndustrialAlcohol industrialAlcohol : getStaticAlcoholForMarket()) {
            applyNPCAlcohol(industrialAlcohol);
        }

        if (market.getFactionId().equals(Factions.INDEPENDENT))
            applyNPCAlcohol(AlcoholRepo.getIndustrial(AlcoholRepo.WATER)); //everyone loves heroin
        if (market.getSize() > 5)
            applyDemand(Industries.POPULATION, AlcoholRepo.getIndustrial(AlcoholRepo.TEA).getPopulationImportMod(), AlcoholRepo.getIndustrial(AlcoholRepo.TEA).getCommodityId()); //tea demand to large worlds
    }

    private void applyNPCAlcohol(IndustrialAlcohol industrialAlcohol) {
        String commodityID = industrialAlcohol.getCommodityId();

        if (!market.hasIndustry(Brewery.INDUSTRY_ID)) {
            applyDemand(Industries.LIGHTINDUSTRY, industrialAlcohol.getLightIndustryMod(), industrialAlcohol.getDemandsForProduction());
            applySupply(Industries.LIGHTINDUSTRY, industrialAlcohol.getLightIndustryMod(), commodityID);
        }

        applyDemand(Industries.POPULATION, industrialAlcohol.getPopulationImportMod(), commodityID);
    }
    ...
    private List<IndustrialAlcohol> getStaticAlcoholForMarket() {
